...
% added: same loosening for single-column floats (figure/table), which
% otherwise defer past \topfraction=0.7 of a column and can drift far
% from where they are cited (e.g. the 20-row placement table).
...
accuracy into prioritised sensor placement guidance for Swedish municipal and
smart-city air quality planning.
\end{abstract}

\begin{IEEEkeywords}
air quality, spatial estimation, machine learning, IoT sensor placement,
smart cities, spatial cross-validation, land-use regression, Sweden
\end{IEEEkeywords}
...
no direct measurement record. This gap is increasingly discussed within the
broader smart city paradigm, where air quality is one of several
environmental parameters a city is expected to sense and act on
continuously: Munera et al. \cite{munera_iot_aq_smart_cities_2021} find in a
systematic mapping study of IoT air quality proposals that monitoring
capability often outpaces its use in actual planning.
...
($a + b\ln d$), power ($a\,d^{b}$), and exponential ($a\,e^{bd}$). Parameters
are estimated by ordinary least squares, regressing RMSE directly for the
logarithmic form and $\ln(\mathrm{RMSE})$ on the transformed distance term
for the power and exponential forms. The form is selected by the Akaike
Information Criterion (AIC) \cite{akaike_1974}, computed from each curve's
residuals in RMSE units so all three forms are compared on a common footing;
this is appropriate because the fitted curve is later evaluated at thousands
of grid-cell distances outside the fitted sample. The reliable prediction
distance $d^{*}$ is defined as the distance at which expected RMSE reaches 50
percent of the sample mean concentration; this working criterion is swept
from 20 to 80 percent in a sensitivity analysis.
...
each estimator. The exponential form is selected for both RF
(AIC~$=25.33$) and IDW (AIC~$=38.55$): IDW error rises from below
1.3~$\mu$g\,m$^{-3}$ within 10~km to about 5.5~$\mu$g\,m$^{-3}$ at 82~km.
Fig.~\ref{fig:decay-no2} shows the corresponding relationship for NO$_2$: the
IDW curve follows a logarithmic rise (AIC~$=63.38$), while the aggregate
NO$_2$ RF curve, also best fit by the logarithmic form (AIC~$=60.68$, only
0.65 ahead of the exponential alternative, a weak-evidence margin), is
inverted by the two rural stations, whose near-zero concentrations suppress
absolute error regardless of distance.
...
71.4~percent of urban area (Fig.~\ref{fig:coverage}), and population-weighted
PM$_{2.5}$ coverage rises
...
Dalarna, and Norrbotten. Table~\ref{tab:placement-coords} lists the 20
recommended locations in priority order.

\begin{table}[!ht]
\caption{Greedy sequential sensor placement: 20 recommended locations in
priority order. Coordinates represent the centre of the selected 10~km grid
cell. Cumulative PM$_{2.5}$ coverage is the percentage of Swedish land cells
within the 64~km reliable estimation threshold after each placement
(baseline: 10.9\%).}
...
\begin{tabular}{@{}rrrrl@{}}
\toprule
\textbf{Rank} & \textbf{Lat. ($^\circ$N)} & \textbf{Lon. ($^\circ$E)} & \textbf{PM$_{2.5}$ cov.\ (\%)} & \textbf{Zone} \\
\midrule
1  & 56.55 & 14.23 & 13.7 & G\"otaland \\
2  & 57.31 & 12.82 & 16.5 & G\"otaland \\
3  & 57.60 & 15.18 & 19.4 & G\"otaland \\
4  & 58.36 & 13.75 & 22.2 & G\"otaland \\
5  & 58.74 & 15.87 & 25.0 & G\"otaland \\
...
reported coverage percentages are contingent on the 50~percent criterion:
sweeping it from 20 to 80~percent swings post-placement PM$_{2.5}$ coverage
from 2.9\% to 94.7\%. What is robust across that range is the
