...
accurate estimator, with a mean RMSE of 2.33 against 3.56 microgram per
...
lowest mean RMSE at 7.27 microgram per cubic metre. Decay curves fitted to
...
ESCAPE project reports cross-validation R$^2$ of about 0.71 for PM$_{2.5}$
\cite{eeftens_escape_pm_2012} and leave-one-out R$^2$ of 0.83 for NO$_2$
\cite{wang_escape_no2_2013}. Machine learning methods are increasingly applied
for their capacity to model non-linear predictor interactions, with reviews
reporting consistent RMSE and MAE improvements over regression baselines
...
rigor guidelines of Hevner et al. \cite{hevner_design_science_2004}.
...
Thirty predictors span five groups: 18 CORINE land-use proportions (nine
...
distance to the nearest retained training station. Three functional forms are
fitted to the 11 resulting pairs per estimator: logarithmic
($a + b\ln d$), power ($a\,d^{b}$), and exponential ($a\,e^{bd}$). The form is
selected by the Akaike Information Criterion (AIC), which is appropriate
because the fitted curve is later evaluated at thousands of grid-cell
distances outside the fitted sample. The reliable prediction distance $d^{*}$
...
Fig.~\ref{fig:decay} shows per-station RMSE against distance to the nearest
retained training station, with the AIC-selected curve for each estimator. For
PM$_{2.5}$ the exponential form is selected for both RF and IDW: IDW error
rises from below 1.3~$\mu$g\,m$^{-3}$ within 10~km to about 5.5~$\mu$g\,m$^{-3}$
at 82~km. For NO$_2$ the IDW curve follows a logarithmic rise, while the
aggregate NO$_2$ RF curve is inverted by the two rural stations, whose
near-zero concentrations suppress absolute error regardless of distance.

Solving the fitted IDW curves at the 50~percent criterion gives an estimated
reliable prediction distance of
...
and $d^{*}_{\mathrm{NO2}} \approx 6$~km from the corresponding logarithmic
model. Both figures are criterion-dependent estimates rather than fixed
properties of the network: sweeping the criterion from 20 to 80~percent of the
sample mean concentration moves $d^{*}_{\mathrm{PM2.5}}$ between roughly 12 and
98~km. The NO$_2$ threshold is further confounded by station type, with only
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_coverage_before_after.png}
...
reported coverage percentages are contingent on the 50~percent criterion: a 20
to 80~percent sweep moves post-placement PM$_{2.5}$ land coverage between 2.9
and 94.7~percent. What is robust across that range is the PM$_{2.5}$/NO$_2$
asymmetry itself.
