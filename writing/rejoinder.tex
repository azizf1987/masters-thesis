...
\begin{enumerate}[label=\arabic*.,leftmargin=*]
  ...
  \item A buffer-distance sensitivity analysis was run: the full buffered SLOO procedure was
    repeated at 0, 5, 10, 25, and 50\,km exclusion radii. The estimator ranking is unchanged at every
    ...
    corresponding sentence is added to the limitations section (\S8.4). Buffers approaching the
...
Agreed, and already stated as a limitation. \S8.4 notes that the 11 concurrently-passing
...
\subsection{E7: Section 6.4 (Estimated Prediction-Distance Threshold and Answer to RQ2)}
...
grid). Results: population-weighted PM$_{2.5}$
...
a caveat is added to \S8.4 noting that the population-weighted NO$_2$ figures inherit the same
...
\rjresponse{Fixed.}
The single existing sentence in \S6.2 (``LUR curves are reported for completeness but carry
...
\rjcomment{\emph{(linnea)} Shouldn't this be a sub-title? \emph{(on the portion of the title
after the colon)}}
...
\rjcomment{\emph{(linnea)} A short introduction to the different topics you talk about in this
...
\rjcomment{\emph{(Parisa)} The section discusses relevant previous studies but the process used to
...
\rjcomment{\emph{(linnea)} This thesis? \emph{(on the phrase ``The present thesis'')}}
...
\rjcomment{\emph{(linnea)} When writing abbreviations the text should be ``Inverse Distance
...
\rjcomment{\emph{(linnea)} Reference? \emph{(on the paragraph describing LUR's known
limitations)}}
...
\rjcomment{\emph{(linnea)} All theses do not need to look exactly the same, but I think
...
\rjcomment{\emph{(Parisa)} Section 4.1 represents that all 34 stations may contribute training
...
appeared incorrectly (\S3.2.2, \S3.2.3, \S8.4), aligning them with \S4.1.

\subsection{O9: Section 5 (Spatial Estimation Models)}

\rjcomment{\emph{(linnea)} What is ``this chapter''? I think the structure of what is results
...
\rjcomment{\emph{(linnea)} This is ``double negative'' phrasing which is a bit unclear, and as
...
\subsection{O11: Section 6.4 (Estimated Prediction-Distance Threshold and Answer to RQ2)}

\rjcomment{\emph{(Parisa)} The sensitivity analysis clearly explains that the 50\,\% criterion is
...
\rjcomment{\emph{(linnea)} The triangles are red not green. A little confusion to write ``placed
...
\rjcomment{\emph{(linnea)} These labels are impossible to see or read. \emph{(on the numbered
...
\rjcomment{\emph{(Parisa)} IEEE reference guide is to write et al.\ after six authors.}
...
E10 & Figures 2--3 LUR centre panel & Fixed (expanded \S6.2 + caption clauses) \\
