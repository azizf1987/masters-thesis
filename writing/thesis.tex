...
Sweden's national air quality reference network, operated by SMHI, produces high-fidelity measurements of PM$_{2.5}$ and NO$_2$ but is spatially sparse, leaving large areas without direct monitoring. This thesis investigates how accurately these pollutants can be estimated at unmonitored locations, how that accuracy degrades with distance from the nearest reference station, and how the resulting relationship can guide the placement of supplementary IoT sensors. Following a Design Science Research methodology, a Random Forest model was trained on land-use, topographic, meteorological, and population covariates for 34 SMHI stations (2020--2024) and evaluated under a buffered spatial leave-one-out cross-validation protocol, benchmarked against inverse distance weighting (IDW) and land-use regression (LUR). Random Forest achieved a mean RMSE of 7.36\,$\mu$g\,m$^{-3}$ for NO$_2$ and 3.56\,$\mu$g\,m$^{-3}$ for PM$_{2.5}$, outperforming LUR for both pollutants, while IDW outperformed Random Forest for PM$_{2.5}$ due to its smoother spatial autocorrelation structure. Fitting decay curves to per-station error against distance yielded reliable prediction distance thresholds of approximately 64~km for PM$_{2.5}$ and 6~km for NO$_2$. Using the PM$_{2.5}$ threshold, a greedy sequential placement algorithm was applied to a national 10~km grid to recommend 20 candidate IoT sensor locations, increasing national PM$_{2.5}$ land-area coverage from 10.9\,\% to 67.2\,\% and urban-area coverage from 38.8\,\% to 71.4\,\%, and extending reliable coverage to four of Sweden's seven currently unmonitored administrative regions. The contribution is an empirically grounded, reproducible artefact that translates spatial estimation accuracy into concrete, prioritised sensor placement guidance for Swedish municipal air quality planning.
...
\begin{enumerate}
    ...
    \item \textbf{RQ3:} Applying the 64~km PM$_{2.5}$ threshold, a greedy sequential placement algorithm identifies 20 candidate IoT sensor locations that raise national PM$_{2.5}$ land-area coverage from 10.9\,\% to 67.2\,\% and urban-area coverage from 38.8\,\% to 71.4\,\%, extending reliable coverage to four of the seven Swedish administrative regions currently without a concurrent-passing reference station.
\end{enumerate}
